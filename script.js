...
function goToChapter(chapterName) {
  ...
  const optionsArr = chapter.options.length;
  let textChoix = " ";

  for (let index = 0; index < optionsArr; index++) {
    const option = chapter.options[index];
    textChoix += `<button class="choix" onclick="${option.action}">${option.text}</button>`;
  }
  document.querySelector(".bouton").innerHTML = textChoix;
}
